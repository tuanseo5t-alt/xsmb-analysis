...
if __name__ == '__main__':
    ...
    results_2d = lottery.get_2_digits_data()
    sparse_results = lottery.get_sparse_data()

    last_date = results['date'].max()

    # Bảng đã được lấy 2 số cuối -> dùng cho phân tích.
    start_date = pd.Timestamp(year=last_date.year - 2, month=last_date.month, day=last_date.day)
    results_2_year = results_2d[(start_date < results_2d['date']) & (results_2d['date'] <= last_date)]
    results_2_year = results_2_year.drop(columns=['date'])
    ...
    small_results = results_2d[(start_date < results_2d['date']) & (results_2d['date'] <= last_date)]
    small_results.reset_index(drop=True, inplace=True)

    # Last appearing Special price
    last_appearing(results_2_year, 'tail_special')

    numeric_cols = [c for c in small_results.columns if c not in ('date', 'thu', 'tinh', 'ngay_text', 'ky_hieu')]
    recent_results = small_results[numeric_cols].iloc[-1].values
    ...
    # Lấy record ngày mới nhất từ raw data (chuỗi zero-padded) để hiển thị README.
    latest_raw = results[results['date'] == results['date'].max()].iloc[0]
    render = Render()
    context = {
    ...
    numeric_cols = [c for c in small_results.columns if c not in ('date', 'thu', 'tinh', 'ngay_text', 'ky_hieu')]
    loto_numeric = small_results[['date'] + numeric_cols]
    last_appearing_loto(loto_numeric)